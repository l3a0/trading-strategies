...
**That 1.63 is a proxy number.** This tutorial prices every option from the `HV × multiplier` proxy, and the README's [Reality check: real option chains](https://github.com/l3a0/covered-call-backtesting/blob/main/README.md#reality-check-real-option-chains) section re-measures the same delta-hedged strategy on ten years of traded MSFT quotes: the Newey-West t-stat collapses to −0.23 at bid/ask fills and +0.73 at mid. The hedge still cuts variance as designed — the premium it was built to isolate just isn't there at real quote levels. The 1.63 was the proxy's inflated premiums talking; `TestMsftRealRiskManagedRegression` pins the real-chain numbers in CI.
