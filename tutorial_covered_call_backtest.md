...
The full Black-Scholes toolkit — `normal_pdf`, `normal_cdf`, `bs_price`, `bs_delta`, and `find_strike_for_delta` — lives in [`cc_backtest.py`'s section 1](https://github.com/l3a0/trading-strategies/blob/main/engine/cc_backtest.py#L14). What each one does:
...
Implementations: [`cc_backtest.py::detect_regime`](https://github.com/l3a0/trading-strategies/blob/main/engine/cc_backtest.py#L164) and [`::estimate_iv`](https://github.com/l3a0/trading-strategies/blob/main/engine/cc_backtest.py#L173).

### Rolling Historical Volatility: 30-Day Window, Log Returns, Annualize

The implementation is [`cc_backtest.py::calc_rolling_volatility`](https://github.com/l3a0/trading-strategies/blob/main/engine/cc_backtest.py#L129) — for each price index, it computes the standard deviation of the last `window` log returns and annualizes by `√252`.
...
The core backtesting engine lives in [`cc_backtest.py::run_cc_overlay`](https://github.com/l3a0/trading-strategies/blob/main/engine/cc_backtest.py#L204). It's heavily commented and small enough to read end-to-end. Function signature:
...
Expanding the grid into all 27 combinations is one Cartesian product — [`cc_backtest.py::_param_combinations`](https://github.com/l3a0/trading-strategies/blob/main/engine/cc_backtest.py#L867) — which `walk_forward_optimization` loops over on each training window.

### How to Stitch Out-of-Sample Results into a Single Equity Curve

The implementation is [`cc_backtest.py::walk_forward_optimization`](https://github.com/l3a0/trading-strategies/blob/main/engine/cc_backtest.py#L881). Signature:
...
The production implementation in [`cc_backtest.py::walk_forward_optimization`](https://github.com/l3a0/trading-strategies/blob/main/engine/cc_backtest.py#L881) is heavily commented. It carries the teaching content (window arithmetic diagram, boolean-indexing explainer, Sharpe-built-inside-out walkthrough, Bessel's correction, √252 annualization derivation, "rules are LOCKED — no re-tuning" emphasis) right next to the code that does the work. The fixing test [`test_cc_backtest.py::TestMsftTenYearRegression::test_walk_forward_optimization`](https://github.com/l3a0/trading-strategies/blob/main/tests/test_cc_backtest.py#L1572) pins the 13 walk-forward periods, the most chosen parameters, and the cumulative OOS compound return on the bundled MSFT data.
...
One accounting note before drawing conclusions: those three numbers don't share a bookkeeping convention. The walk-forward \~324% restarts each 6-month test window at the same $100K and compounds the 13 per-window returns; the fixed-params \~378% and buy-and-hold \~317% each run once, straight through the span. Recomputing all three on one consistent convention changes the sizes but not the ordering. Fixed params stay \~47–52 pp ahead of walk-forward. The walk-forward margin over buy-and-hold widens from the \~7 pp quoted below to \~19 pp (restarting everything) or \~44 pp (carrying capital forward, where reinvested gains do most of the stretching). Retention stays \~86–87%. That swing is the real lesson: endpoint gaps move with accounting choices, so no endpoint gap is the edge measurement. Part 5's Newey-West t-stat on the daily excess series is the measurement, and no choice of convention touches it.
...
The implementation is [`cc_backtest.py::monte_carlo_shuffle`](https://github.com/l3a0/trading-strategies/blob/main/engine/cc_backtest.py#L1178) — it computes daily returns from the real prices, shuffles their order with a fixed seed, rebuilds a synthetic price path from each shuffled sequence, runs the overlay backtest on each, then compares the real ordered path's total return against the distribution. The fixing test [`test_cc_backtest.py::TestMsftTenYearRegression::test_monte_carlo_shuffle`](https://github.com/l3a0/trading-strategies/blob/main/tests/test_cc_backtest.py#L1468) pins the resulting percentile, MC mean, and best-shuffled return on the bundled MSFT data (500 paths, `seed=42`, `__main__` params).
...
The implementation is [`cc_backtest.py::sensitivity_analysis`](https://github.com/l3a0/trading-strategies/blob/main/engine/cc_backtest.py#L1259) — it sweeps `call_delta` and `close_at_pct` at ±0.05 / ±0.10 / ±0.20 offsets from base. The algorithm: hold all params fixed except one, vary that one by a small offset in both directions, measure each variant's total return, then compute the worst drop from base. The fixing test [`test_cc_backtest.py::TestMsftTenYearRegression::test_sensitivity_perturbations`](https://github.com/l3a0/trading-strategies/blob/main/tests/test_cc_backtest.py#L1426) pins each variant's return and asserts the worst drop from base stays under 10% (the "robust" verdict) on the bundled MSFT data — the notebook companion calls the same function, so the two can't drift.
...
The full implementation is [`cc_backtest.py::excess_over_buy_hold_statistics`](https://github.com/l3a0/trading-strategies/blob/main/engine/cc_backtest.py#L612). Signature:
...
**The shortcut.** There's a useful shortcut buried in the math: **t-stat ≈ Sharpe × √(years)**. You can sanity-check the relationship in one line: 0.126 × √10 ≈ 0.40, almost exactly the naive t-stat of 0.40 that `excess_over_buy_hold_statistics` returns. (The Newey-West-adjusted t-stat of 0.46 includes the autocorrelation correction from earlier in this section, which the shortcut doesn't model.) If your Sharpe and your sample length don't multiply to a healthy t-stat, no amount of fiddling with the strategy will rescue it — you need a bigger effect or more data.
...
The t-stat still doesn't clear the t = 2 bar. It's the single-stock VRP weakness from the previous section reasserting itself. The risk-managed version is the correct *measurement* of whether the volatility risk premium is showing up. On the engine's modeled premiums it's showing up, just not at index-CC magnitudes. The honest path from here is what the academic literature already does: same risk-managed framework, run on SPX or SPY, over decades.

**That 1.63 is a proxy number.** This tutorial prices every option from the `HV × multiplier` proxy, and the README's [Reality check: real option chains](https://github.com/l3a0/trading-strategies/blob/main/README.md#reality-check-real-option-chains) section re-measures the same delta-hedged strategy on ten years of traded MSFT quotes: the Newey-West t-stat collapses to −0.23 at bid/ask fills and +0.73 at mid. The hedge still cuts variance as designed — the premium it was built to isolate just isn't there at real quote levels. The 1.63 was the proxy's inflated premiums talking; `TestMsftRealRiskManagedRegression` pins the real-chain numbers in CI.
...
- ✅ Sharpe ratio vs cash (**rf** = risk-free rate; 4.5% is the engine default — see the `risk_free_rate` parameter in [`run_cc_overlay`](https://github.com/l3a0/trading-strategies/blob/main/engine/cc_backtest.py#L204)): \~1.12, vs buy-and-hold MSFT's \~0.72 over the same window — risk-adjusted *absolute* returns are strong
...
10. **Entry trend filter:** Add a parameterized SMA trend filter (gate entries on SMA50 vs. SMA200) as an on/off axis in the walk-forward grid. Today the engine ships no trend filter by design. The CC-phase argument (selling into downtrends is desirable for call sellers — premium collected, cost basis reduced) predicts it won't help on the call side. That prediction was tested in June 2026 by a pre-registered experiment ([registration](docs/prereg_trend_gate.md), [results](docs/trend_gate_results.md)): both mechanism checks failed with the wrong sign — cycles entered during uptrends did *better*, not worse — and the registered verdict is null, validating the no-trend-filter design empirically. The grid-axis variant sketched here was reserved for a separate registration and remains unrun.
...
| --- | --- |
| [`cc_backtest.py`](https://github.com/l3a0/trading-strategies/blob/main/engine/cc_backtest.py#L204) | Black-Scholes pricing, rolling-volatility helpers, the [`run_cc_overlay`](https://github.com/l3a0/trading-strategies/blob/main/engine/cc_backtest.py#L204) engine, and [`excess_over_buy_hold_statistics`](https://github.com/l3a0/trading-strategies/blob/main/engine/cc_backtest.py#L612) for Newey-West t-stats |
| [`test_cc_backtest.py`](https://github.com/l3a0/trading-strategies/blob/main/tests/test_cc_backtest.py#L38) | Pytest suite covering pricing primitives, the overlay state machine, scenario tests, and the statistics helper |
| [`download_prices.py`](https://github.com/l3a0/trading-strategies/blob/main/pipeline/download_prices.py#L13) | Fetches historical daily closes via yfinance |
...
| [`requirements.txt`](https://github.com/l3a0/trading-strategies/blob/main/requirements.txt) | Pinned dependencies |
| [`README.md`](https://github.com/l3a0/trading-strategies/blob/main/README.md) | Quick-start instructions and project summary |

**To run it locally:**

```bash
git clone https://github.com/l3a0/trading-strategies.git
cd trading-strategies
pip install -r requirements.txt
python -m engine.cc_backtest            # runs the backtest on bundled MSFT data
