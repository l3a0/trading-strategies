...
# the exact sell-side line from engine/cc_backtest.py#L336:
...
from engine.cc_backtest import _param_combinations
...
# `mc` already came from engine.cc_backtest.monte_carlo_shuffle in the setup
...
from engine.cc_backtest import sensitivity_analysis
...
from engine.cc_backtest import regime_analysis
...
if "google.colab" in sys.modules:
    if not os.path.isdir("trading-strategies"):
        subprocess.run(
            ["git", "clone",
             "https://github.com/l3a0/trading-strategies.git"],
            check=True,
        )
    os.chdir("trading-strategies")
    subprocess.run(
...
from engine.make_figures import (
    fig1_equity_curves,
...
dates, prices = load_msft_csv("data/msft_10yr_prices.csv")
params = {
    "call_delta": 0.25,
    "close_at_pct": 0.75,
    "dte": 21,
...
# Walk-forward grid mirrors tests/test_cc_backtest.py's pinned grid.
param_grid = {
    "call_delta": [0.15, 0.20, 0.25],
    "dte": [21, 30, 45],
    "close_at_pct": [0.50, 0.75, 1.00],
...
[`tutorial_covered_call_backtest.md`](https://github.com/l3a0/trading-strategies/blob/main/tutorial_covered_call_backtest.md)
by [`make_notebook.py`](https://github.com/l3a0/trading-strategies/blob/main/engine/make_notebook.py)
