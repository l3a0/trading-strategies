...
The loading regression is a plain OLS through the shared `ithildincore.timeseries.ols`
...
def _ols_tstats(y: np.ndarray, x: np.ndarray) -> np.ndarray:
    ...
    via the shared `ithildincore.timeseries.ols` (statsmodels-backed). Returns the
