...
- **Publishing:** `gzip -k -9` the CSV → append its sha256 to `data_checksums.sha256` → `gh release upload data-2026-06 <file>.gz` → add the filename to the CI cache `path` list in `ci.yml` (the fetch glob `*_option_dailies.csv.gz` already matches new files; the cache key auto-rotates because it hashes the checksums file) → round-trip verify by downloading the asset fresh and running `shasum -a 256 -c` → copy the `.gz` + checksums file to the personal cold-storage folder (location lives in private memory, deliberately not in this file — CLAUDE.md is carried by the public mirror).
