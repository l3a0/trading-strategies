...
| [read_gate_wire.py](read_gate_wire.py) | The dependency-free read-gate seal contract (`WIRE_VERSION`, `BANNED_RESULT_FIELDS`, `assert_numberless`, the coordinate/model field lists) — one source of truth for the in-process oracle seam (`edge_search.score_and_record`) and a future in-process LLM author, kept engine-free so the contract is shareable and testable in isolation (docs/read_gate.md) |
