...
| [docs/llm_proposer_plan.md](docs/llm_proposer_plan.md) | The item-4 (LLM proposer author) design, recorded before any build — the Claude call lives oracle-side; why that trades the container's kernel isolation for a *correctness argument* for the LLM; the prompt-builder leak surface and its three location options — decided: the oracle-side builder, with `assert_numberless` on the assembled prompt as the load-bearing seal; and the unsolved training-leak (post-cutoff time-axis-holdout) blocker that keeps LLM survivors exploratory. Design only — nothing activated |
