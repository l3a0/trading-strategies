...
| [read_gate_wire.py](read_gate_wire.py) | The dependency-free read-gate wire contract (`WIRE_VERSION`, `BANNED_RESULT_FIELDS`, `assert_numberless`, the field lists) shared by the trusted oracle (`edge_search.score_and_record`) and the future sandboxed proposer — one source of truth, so neither side vendors a drifting copy (docs/read_gate.md) |
