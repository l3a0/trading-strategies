...
| [edge_search.py](edge_search.py) | Automated FDR-controlled edge-search harness — two phases over one Benjamini-Yekutieli ledger + sealed vault: the cheap **re-tag** class (entry-conditioning templates, MSFT+SPY, QQQ sealed) and the **engine-re-run** class (`run_structure_campaign` — short-vol/straddle/condor overlays across six tickers scored by `short_vol_statistics`' HAC-t asymptotic null, TLT sealed) (`python edge_search.py [structure]`) |
| [test_edge_search.py](test_edge_search.py) | Pins both campaigns (Campaign 1 re-tag + Campaign 2 structure — neither has a survivor under BY) plus always-run FDR / enumerator / kill-gate / seal / scale-guard logic |
