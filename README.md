...
| [docs/alphaagent_integration_plan.md](docs/alphaagent_integration_plan.md) | How to fold AlphaAgent's good ideas (hypothesis-first generation + hypothesis-factor alignment) into the proposer **without** its leak (the Eval-Agent backtest→refine loop = in-loop p-hacking). The integration is additive: a numberless `Hypothesis` schema + an oracle-side alignment gate (`derive_family` must match the *claimed* premium family) on top of the existing seal / e-LOND / dedup. Includes a current-vs-new diagram, the agent mapping, and the library-reuse declaration (internal stack as-is + anthropic; Qlib/AlphaGen/gplearn/alphalens only if extending to factors). Design only — nothing built; promotion still gated on the Phase-C holdout |
