...
| [generative_engine.py](generative_engine.py) | **Phase 2** engine layer of the generative grammar: `derive_family` types an arbitrary composition's mechanism into a `PremiumFamily` by RULE over the engine's robust signature axes (`net_vega` / `net_delta` / `net_skew` / `expirations`) — the generative replacement for the per-overlay family table, returning `None` (fail-closed) for a mechanism-incoherent signature. Reproduces all 7 declared families. The byte-identical composer + equivalence proof are the next increment |
| [test_generative_engine.py](test_generative_engine.py) | Always-run pins for `derive_family`: it reproduces the 7 committed overlays' declared families, reaches every registered family, fails closed on an incoherent signature, and respects the TERM-priority / skew-needs-neutral-vega rules |
