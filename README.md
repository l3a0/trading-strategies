...
| [validate_dailies.py](validate_dailies.py) | Data-hygiene validator + `CHAIN_CLEAN_START` proposer — streams a chain store, flags the placeholder-greeks era (constant-mark / lattice-IV / step-delta rows that leave the entry band defect-free), and proposes the clip or fails closed to "UNVERIFIED"; calibrated to reproduce MSFT's `2010-05-10` (`python validate_dailies.py GLD TLT XLE EEM`) |
| [test_validate_dailies.py](test_validate_dailies.py) | Always-run synthetic pins of the classifier + boundary scan, plus dataset-gated calibration (MSFT backfill → `2010-05-10`; GLD/TLT/XLE/EEM → CLEAN, no clip) |
