...
| [oracle_server.py](oracle_server.py) | Read-gate transport — the TRUSTED oracle: an NDJSON `serve` loop wrapping `score_and_record` (records before replying, returns only the one-bit scoreboard) + `launch`/`prepare_sandbox` that spawn a sandboxed proposer (fail-closed `assert_sandbox_clean`, scrubbed env). The supervised-operator MVP — a same-machine cwd, NOT a kernel jail; the container is the remaining hardening (docs/read_gate.md) |
...
| [Dockerfile.proposer](Dockerfile.proposer) | The read-gate **sealed proposer image** — bakes in only the two engine-free files (`proposer_client.py` + `read_gate_wire.py`), no engine/chains/ledger. Run under the hardened seal it makes the engine + answer key kernel-unreachable (mount namespace) and the network dead (`--network none`). The launch-uses-it integration is a later PR (docs/read_gate.md) |
| [test_read_gate_container.py](test_read_gate_container.py) | Docker-gated CI proof of the container seal (`TestProposerImageSeal`): inside the image the engine is unimportable, the engine source + answer-key ledger are unreadable by host abspath, the network is unreachable, and the proposer's own code still runs engine-free. Skips where docker is absent (the dev box); CI fails if docker is missing rather than silently skipping |
