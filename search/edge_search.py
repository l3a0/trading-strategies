...
def _put_chain_paths(ticker: str) -> list[str]:
    ...
    import os
    base = data_path(f'{ticker.lower()}_option_dailies_puts.csv')
...
def _far_chain_paths(ticker: str) -> list[str]:
    ...
    import os
    base = data_path(f'{ticker.lower()}_option_dailies_180dte.csv')
    return [base] if (os.path.exists(base) or os.path.exists(base + '.gz')) else []
...
def _ticker_scale_ratio(loaded: tuple[Any, list[str], list[float]]) -> float | None:
    ...
    from pipeline.validate_dailies import scale_ratio
    store, dates, prices = loaded
...
def run_structure_campaign(campaign: Campaign = STRUCTURE_CAMPAIGN,
    ...
    from pipeline.validate_dailies import SCALE_TOL
...
PROVENANCE_PATH = data_path('proposal_provenance.jsonl')


def _read_data_checksums(path: str | None = None) -> dict[str, str]:
    ...
    if path is None:
        path = data_path('data_checksums.sha256')
    out: dict[str, str] = {}
    try:
...
def _is_onboarded(ticker: str) -> bool:
    ...
    import os
    base = data_path(f'{ticker.lower()}_option_dailies.csv')
...
# The two LLM-author TRANSPORTS now live in the dependency-free `proposer_clients` (shared with the factor
# author); these thin subclasses bind the OPTION prompt builder, preserving this module's public API + tests.
class ClaudeProposer(_ClaudeApiBase):
    """The OPTION proposer's API client — `proposer_clients.ClaudeProposer` bound to `build_proposer_prompt`
    (the seal gold-standard; see the base class for the activation + numberless-seal contract)."""

    def __init__(self, model: str = 'claude-opus-4-8', **kw: Any) -> None:
        super().__init__(model, prompt_builder=build_proposer_prompt, **kw)


class ClaudeCodeProposer(_ClaudeCodeBase):
    """The OPTION proposer's subscription transport — `proposer_clients.ClaudeCodeProposer` bound to
    `build_proposer_prompt` (see the base class for the hardened `claude -p` invocation seal)."""
