...
if [ "$2" != "--publish" ]; then
  ...
  "${0:A:h}/fetch_batch.sh" $TK || { echo "[ERR] fetch failed" >&2; exit 1; }
  echo ""
  $PY -m pipeline.validate_dailies $TK
  echo ""
  echo "================= SIGN-OFF GATE ================="
  echo "Review the verdict + the price-vs-chain scale line above, then:"
  echo "  CLEAN       -> ./scripts/onboard_ticker.sh $TK --publish"
  echo "  CLIP at D   -> add CHAIN_CLEAN_START['$TK']='D' to realchains/real_cc_backtest.py, then --publish"
  echo "  UNVERIFIED  -> investigate before publishing (the validator fails closed for a reason)"
  exit 0
fi

echo "== phase 2: publish $TK =="
"${0:A:h}/publish_dailies.sh" $TK || { echo "[ERR] publish failed" >&2; exit 1; }
echo ""
echo "== campaign smoke test: structure overlays on $TK (scale-guarded) =="
$PY - "$TK" <<'PYEOF'
import sys
from search.edge_search import run_structure_campaign, Campaign
tk = sys.argv[1]
